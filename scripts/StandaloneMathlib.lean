...
Reader-facing modules in `FKSEdgeThreshold.Standalone.Mathlib` state results so that a reader can
...
private def standaloneMathlibDir : System.FilePath := "FKSEdgeThreshold" / "Standalone" / "Mathlib"
...
private def forbiddenRoots : Array Name := #[`FKSEdgeThreshold, `scripts]
