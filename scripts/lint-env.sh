...
# Mathlib v4.33.1 environment-linter defaults, minus docBlame. This list is compared against
# the live set below and the lint refuses to run when they differ, so a Mathlib bump that
...
[ "${mods:-0}" -gt 0 ] || fail "found no FKSEdgeThreshold/*.lean modules — the lint is miswired"
...
run_meta do
  ...
  for idx in [0:modNames.size] do
    ...
    if let some m := modNames[idx]? then
      if m == \`FKSEdgeThreshold || (\`FKSEdgeThreshold).isPrefixOf m then
...
#lint only $LINTERS in FKSEdgeThreshold
...
if [ -s "$TMP/nolints-new.txt" ]; then
  echo "lint-env: @[nolint <linter>] application(s) under FKSEdgeThreshold/ not accounted for in $ALLOWLIST (as '<linter> <declaration>'):"
...
awk -v pfx="$DRIVER:" '
  BEGIN { hdr = "-- Found [0-9]+ errors? in [0-9]+ declarations \\(plus [0-9]+ automatically generated ones\\) in FKSEdgeThreshold with [0-9]+ linters$" }
  {
    if (index($0, pfx) == 1) {
...
hdr_nlinters=$(sed -n 's/^.* in FKSEdgeThreshold with \([0-9]*\) linters$/\1/p' "$TMP/headers.txt")
