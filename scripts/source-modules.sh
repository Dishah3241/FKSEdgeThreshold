...
# Shared, fail-closed discovery of the Lean modules under FKSEdgeThreshold/.
#
...
#
# The lakefile glob `FKSEdgeThreshold.*` is authoritative for what is built; this function is the
...
primal_source_modules() {
  ...
  local module_path_re="^FKSEdgeThreshold(/[A-Za-z_][A-Za-z0-9_']*)+\.lean$"
  local file module
  ...
  find FKSEdgeThreshold -type f -name '*.lean' -print0 | LC_ALL=C sort -z > "$files_out"
  find FKSEdgeThreshold -type l -print0 | LC_ALL=C sort -z > "$symlinks_out"
  ...
  if [ ! -s "$files_out" ]; then
    echo 'source-modules: found no FKSEdgeThreshold source files; the audit is miswired.' >&2
    return 1
  fi
